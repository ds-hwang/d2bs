...
Script::Script(string file) :
		...
		script(NULL), execCount(0), scriptExecState(ScriptStateCreation),
		...
		trapArg(NULL)
{
	...
	fileName = file;
	shortFileName = fileName.substr(strlen(Vars.szScriptPath));
	std::transform(fileName.begin(), fileName.end(), fileName.begin(), tolower);
	replace(fileName.begin(), fileName.end(), '/', '\\');
	trapHandle = CreateEvent(NULL, TRUE, FALSE, NULL);
