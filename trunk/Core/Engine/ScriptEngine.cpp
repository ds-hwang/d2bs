...
Script* ScriptEngine::FindByName(const char* name)
{
	FindHelper helper = {0, name};
	ForEachScript(FindScriptByName, &helper, 1);
	return helper.script;
}

Script* ScriptEngine::FindByTid(int id)
{
	FindHelper helper = {id};
	ForEachScript(FindScriptByTid, &helper, 1);
	return helper.script;
}
	...
	for(uintN i = 0; i < len; i++)
	{
		switch(format[i])
		{
			...
			default:
			{
				...
				UNREFERENCED_PARAMETER(api_usage_error);
...
bool __fastcall FindScriptByName(Script* script, void* argv, uint argc)
{
	...
	if(script->GetShortFilename() == helper->name)
	{
