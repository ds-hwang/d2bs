...
void InitSettings(void)
{
	...
	GetPrivateProfileString("settings", "DeveloperMode", "false", devMode, 6, fname);
	...
	Vars.bDevMode = StringToBool(devMode);
...
bool ProcessCommand(const char* command, bool unprocessedIsCommand)
{
	...
	/*else if(_strcmpi(argv, "bp") == 0 && Vars.bDevMode)
	{
		const char* type = command+3;
		const char* script = strchr(type, ' ')+1;
		const char* line = strchr(script, ' ')+1;
		if(strlen(type) > 0 && strlen(script) > 0 && strlen(line) > 0)
		{
			uintN ln = atoi(line);
			Script* trapee = ScriptEngine::FindByName(script);
			if(trapee)
			{
				JSContext* cx = trapee->GetContext();
				JSScript* script = trapee->GetJSScript();
				if(ln >= 0 && ln <= JS_GetScriptLineExtent(cx, script))
				{
					if(_strcmpi(type, "add") == 0) JS_SetTrap(cx, script, JS_LineNumberToPC(cx, script, ln), trapHandler, NULL);
					else if(_strcmpi(type, "rm") == 0) JS_ClearTrap(cx, script, JS_LineNumberToPC(cx, script, ln), NULL, NULL);
				}
			} else Print("Script '%s' not found!", script);
		}
	}*/
