...
{
	print("ÿc2NTSB ÿc1:: ÿc3Doing a quick shop.");
	...
	{
		var _count;
		print("ÿc2NTSB ÿc1:: ÿc3Got items!");
		...
		for(_count = 0;_count < _items.length;_count++)
		{
			if(njipCheckGoodItem(_items[_count], NJIP_CHECK_SIMPLE) != 0 && (_items[_count].classid < 587 || _items[_count].classid > 596))
			{
				...
				{
					print("ÿc2NTSB ÿc1::ÿc3 " + _items[_count].name + "ÿc2 -> item was good!");
					NTC_Delay(500);
					
					if(!NTT_ShopItem(_items[_count],npc,3))
					{
						print("ÿc2NTSB ÿc1::ÿc1 Error while shopping!");
						return false;
					} 
					else 
					{
				...
				{
					print("ÿc2NTSB ÿc1::ÿc1 Not enough space to buy item :(!");
				}
			}
		}
	}
	...
	{
		print("ÿc2NTSB ÿc1::ÿc3 Error getting items...");
		return false;
	}
	
	print("ÿc2NTSB ÿc1:: ÿc3Done.");
	return true;
}