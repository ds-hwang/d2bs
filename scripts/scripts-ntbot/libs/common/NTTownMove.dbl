...
function NTTM_LeadNPC(npc, destination, nodedistance, waitdistance, waitdelay, retries, enddistance)
{
	...
	{
		print('ÿc1NTTM_LeadNPC() Error: You\'re D2BS '+ version() +' does not have a moveNPC() function');
		return false;
	}
	...
	{
		print('ÿc1NTTM_LeadNPC() Error: Function requires at least the first two required arguments');
		return false;
	}
	
	if(!npc.isD2BSUnit && !(destination.isD2BSUnit || (destination.isObject && destination.x && destination.y)))
	{
		print('ÿc1NTTM_LeadNPC() Error: Invalid \'npc\' or \'destination\' object provided, or object is now out of range');
		return false;
	}
	...
	while(getDistance(npc, dest.x, dest.y) > enddistance)
	{
		...
		{
			print('ÿc1NTTM_LeadNPC() Error: Path failed to generate');
			return false;
		}
		else
		{
			...
			{
				print('ÿc1NTTM_LeadNPC() Error: Failed to move to path point ('+ nextx +', '+ nexty +')');
				return false;
			}
		}
